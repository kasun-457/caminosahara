...
async function saveDetailPanel() {
  ...
  act.time = time; act.endTime = endTime; act.category = category; act.title = title; act.notes = notes; act.details = details;
  ...
  unsubscribeTrips = db.collection('trips')
    ...
    .onSnapshot(snapshot => {
      ...
      if (currentTripId) {
        const trip = trips.find(t => t.id === currentTripId);
        if (trip) {
          if (calView === 'list') renderDayTabs(trip);
...
// ── 일정 모달 ─────────────────────────────────────────────────────────────────
function openActivityModal(activityId, date, presetTime) {
...
function renderGridView(trip) {
  ...
  if (calView === 'all') {
    ...
    const count = calView === '3day' ? 3 : 1;
    calDateOffset = Math.max(0, Math.min(calDateOffset, allDates.length - count));
    visibleDates = allDates.slice(calDateOffset, calDateOffset + count);
  }
  ...
  calEl.querySelectorAll('.cal-event').forEach(el => {
    el.addEventListener('click', e => { if (e.target.closest('.cal-event-resize')) return; openDetailPanel(el.dataset.id, el.dataset.date); });
  ...
  calEl.querySelectorAll('.cal-column').forEach(col => {
    col.addEventListener('click', e => {
      ...
      openActivityModal(null, col.dataset.date, timeStr);
    });
  });
}
...
function updateCalPeriodLabel(trip, visibleDates, allDates) {
  ...
  if (prevBtn) prevBtn.disabled = calDateOffset <= 0 || calView === 'all';
  if (nextBtn) nextBtn.disabled = calDateOffset + count >= allDates.length || calView === 'all';
}
...
function setupEventDrag(eventEl, calBody) {
  ...
  function onMove(e) {
    if (!state) return;
    const relY = e.clientY - state.bodyRect.top + calBody.scrollTop - state.offsetY;
    const snapped = Math.round(relY / (HOUR_PX / 4)) * (HOUR_PX / 4);
    const clamped = Math.max(0, Math.min(snapped, HOUR_PX * 24 - state.height));
  ...
  async function onUp() {
    ...
    const trip = trips.find(t => t.id === currentTripId);
    const act = trip?.days.find(d => d.date === state.date)?.activities.find(a => a.id === state.actId);
    ...
    await updateActivityFields(state.actId, state.date, updates);
    state = null;
  }
}
...
function setupEventResize(resizeEl, calBody) {
  ...
  function onMove(e) {
    ...
    const newH = Math.max(HOUR_PX / 4, state.startH + delta);
    const snapped = Math.round(newH / (HOUR_PX / 4)) * (HOUR_PX / 4);
    state.eventEl.style.height = snapped + 'px';
    const topMin = pxToMinutes(parseInt(state.eventEl.style.top));
    const endMin = topMin + pxToMinutes(snapped);
    const timeEl = state.eventEl.querySelector('.cal-event-time');
  ...
  async function onUp() {
    ...
    if (!state) return;
    const topMin = pxToMinutes(parseInt(state.eventEl.style.top));
    const endMin = topMin + pxToMinutes(parseInt(state.eventEl.style.height));
    await updateActivityFields(state.actId, state.date, { endTime: minutesToTimeStr(endMin) });
    state = null;
  }
}
...
function calNavigate(dir) {
  ...
  if (!trip) return;
  const allDates = getDays(trip.startDate, trip.endDate);
  const step = calView === '3day' ? 3 : 1;
  calDateOffset = Math.max(0, Math.min(calDateOffset + dir * step, allDates.length - step));
  renderGridView(trip);
}
